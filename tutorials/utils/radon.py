...
class TransformRadonTutorialData(etl.Transform):
    ...
    def _tx_radon_df(self, df: DataFrame) -> DataFrame:
        ...
        df['county'] = df['county'].str.strip().replace(
            {
                'ST LOUIS': 'ST. LOUIS',
...
def log_plot_comparison(data: pd.Series, nbins: int = 40):
    ...
    log_kde.fit()
    log_scaled_density = (
        (log_kde.density / log_kde.density.max()) * log_histogram.max()
    )
    ...
    radon_floor_source = ColumnDataSource(
        {
            'x': (
                df['floor'].values
                + np.random.normal(scale=0.02, size=df.shape[0])
            ),
...
def sample_county_trace_ranks(sample_counties, alphas):
    colors = ['#2a2eec', '#fa7c17', '#328c06', '#c10c90']
    plots = []
...
def uranium(summary_df, df):
    ...
    g = summary_df.loc[
        summary_df.index.astype(str).str.startswith('gamma'),
        'mean'
    ].values
    y = a + g * x
    plot.line(
