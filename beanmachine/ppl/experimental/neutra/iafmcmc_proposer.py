...
class IAFMCMCProposer(AbstractSingleSiteProposer):
    ...
    def propose(self, node: RVIdentifier, world: World) -> Tuple[Tensor, Tensor, Dict]:
        ...
        key_ = tuple(f_z[-1].squeeze().detach().numpy().tolist())
        self.sample_dict_[key_] = x
    ...
    def post_process(
        ...
        if (
            old_node_var is not None
            and old_value is not None
            and len(self.sample_dict_) > 1
        ):
            old_transform_value = old_node_var.transform_value(old_value)
            key_ = tuple(old_transform_value.detach().numpy().tolist())
            x_ = self.sample_dict_[key_]
        else:
            return tensor(0.0)
        old_jacobian = old_node_var.jacobian
        f_z, elbo, ja = self.mapping(x_)
        old_proposed_log_prob = (
            dist.Normal(0.0, 1.0).log_prob(x_).sum() + ja.sum() + old_jacobian
        )
