...
class PredictiveTest(unittest.TestCase):
    ...
    @unittest.skip("Automatic broadcasting not yet implemented.")
    def test_predictive_data(self):
        x = torch.randn(4)
        y = torch.randn(4) + 2.0
        obs = {self.likelihood_reg(x): y}
        post_samples = bm.SingleSiteAncestralMetropolisHastings().infer(
            [self.prior()], obs, num_samples=10, num_chains=2, vectorized=True
        )
        assert post_samples[self.prior()].shape == (2, 10)
        test_x = torch.randn(4)
        test_query = self.likelihood_reg(test_x)
        predictives = bm.simulate([test_query], post_samples)
        assert predictives[test_query].shape == (2, 10, 4)
