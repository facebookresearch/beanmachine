...
TEST(testdistrib, normal) {
  ...
  auto half_cauchy_dist = g.add_distribution(
      DistributionType::HALF_CAUCHY, AtomicType::POS_REAL, std::vector<uint>{pos_scale});
  ...
  auto normal_dist3 = g.add_distribution(
      DistributionType::NORMAL, AtomicType::REAL, std::vector<uint>{real1, pos_sq_val});
  auto real_val3 = g.add_operator(OperatorType::SAMPLE, std::vector<uint>{normal_dist3});
  g.observe(real_val3, 5.0);
  grad1 = grad2 = 0;
  g.gradient_log_prob(pos_val, grad1, grad2);
  // f(x) = -log(pi/2) + log(3.5) - log(3.5^2 + x^2) -0.5 log(2pi) -log(x^2) -0.5*(5 + 11)^2/x^4
  // f'(x) = -2x/(3.5^2 + x^2) -2/x +0.5*16^2*4/x^5
  // f'(7) = -0.483822
  // f''(x) = -2/(3.5^2 + x^2) + 4x^2/(3.5^2 + x^2)^2   +2/x^2 -0.5*16^2*4*5/x^6
  // f''(7) = 0.038648
