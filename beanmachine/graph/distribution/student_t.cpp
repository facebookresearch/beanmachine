...
StudentT::StudentT(
  ...
  if (in_nodes[0]->value.type != graph::AtomicType::POS_REAL
      ...
      or in_nodes[2]->value.type != graph::AtomicType::POS_REAL) {
    throw std::invalid_argument(
  ...
  if (sample_type != AtomicType::REAL) {
    throw std::invalid_argument("StudentT distribution produces real number samples");
  }
}
...
// d2f / dl2 = - (n+1) / (n s^2 + (x-l)^2) + 2 (n+1) (x-l)^2 / (n s^2 + (x-l)^2)^2
// df / ds  = -1/s -(n+1) ( n s / (n s^2 + (x-l)^2)  - 1/s )
// d2f ds2  = 1/s^2 -(n+1)( n / (n s^2 + (x-l)^2) - 2 n^2 s^2 / (n s^2 + (x-l)^2)^2  + 1/s^2)
...
void StudentT::gradient_log_prob_param(
  ...
  const double n_s_sq_p_x_m_l_sq = n * s * s + (x - l) * (x -l); // n s^2 + (x-l)^2
  ...
  if (n_grad != 0 or n_grad2 != 0) {
    double grad_n = 0.5*util::polygamma(0, (n+1)/2) - 0.5*util::polygamma(0, n/2) - 0.5/n
      - 0.5 * (std::log(n_s_sq_p_x_m_l_sq) - std::log(n) - 2 * std::log(s))
      - 0.5 * (n+1) * (s*s / n_s_sq_p_x_m_l_sq - 1/n);
    double grad2_n2 = 0.25*util::polygamma(1, (n+1)/2) - 0.25*util::polygamma(1, n/2) + 0.5 / (n*n)
      - (s*s / n_s_sq_p_x_m_l_sq - 1/n)
      -0.5 * (n+1) *( - std::pow(s,4) / (n_s_sq_p_x_m_l_sq*n_s_sq_p_x_m_l_sq) + 1 / (n*n));
    grad1 += grad_n * n_grad;
  ...
  if (l_grad != 0 or l_grad2 != 0) {
    ...
    double grad2_l2 = -(n+1) / n_s_sq_p_x_m_l_sq
      + 2 * (n+1) * (x-l)*(x-l) / (n_s_sq_p_x_m_l_sq * n_s_sq_p_x_m_l_sq);
    grad1 += grad_l * l_grad;
  ...
  if (s_grad != 0 or s_grad2 != 0) {
    ...
    double grad2_s2 = 1/(s*s) -(n+1)*(n/n_s_sq_p_x_m_l_sq
      - 2 * n*n * s*s / (n_s_sq_p_x_m_l_sq * n_s_sq_p_x_m_l_sq) + 1 /(s*s));
    grad1 += grad_s * s_grad;
