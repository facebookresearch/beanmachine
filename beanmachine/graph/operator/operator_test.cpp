...
TEST(testoperator, logsumexp) {
  ...
  // f_xz = dist.Normal(in_nodes.logsumexp(dim=0),tensor(1.0)).
  //   log_prob(tensor(0.0))
...
TEST(testoperator, log) {
  Graph g;
  // negative tests: exactly one pos/tensor should be the input
  EXPECT_THROW(
...
TEST(testoperator, pow) {
  ...
  EXPECT_THROW(
      ...
      std::invalid_argument);
  // Base must be prob/pos/real/tensor.
  // Power must be pos/real/tensor.
  EXPECT_THROW(
