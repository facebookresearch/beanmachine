...
print(BMGInference().to_dot(
    [theta_0(), theta_1()] + x_rvs + y_rvs, {}
))
test_samples = BMGInference().infer(
    [theta_0(), theta_1()] + x_rvs + y_rvs, {}, 1, num_chains=1,
)
...
samples = BMGInference().infer(queries, observations, num_samples, num_chains=1)

inferred_intercept = samples[theta_0()].mean()
inferred_slope = samples[theta_1()].mean()
print(inferred_intercept, inferred_slope)
