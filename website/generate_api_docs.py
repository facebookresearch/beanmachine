...
Sphinx output with Docusaurus v2.
...
any working directory since it uses the relative path to the beanmachine
module.
...
if __name__ == "__main__":
    ...
    try:
        os.makedirs(os.path.join(beanmachine.__path__[0], "../../website/docs/api"))
    except OSError as e:
...
collapsed: {'false' if mod_name in module_hierarchy.keys() else 'true'},\
  items: [{', '.join(items)}],\n}}"
    ...
    for name, entity in name_entity_mapping.items():
        ...
        with open(os.path.join(beanmachine.__path__[0], filename), "w") as file:
