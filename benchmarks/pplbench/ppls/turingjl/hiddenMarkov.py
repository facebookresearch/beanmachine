...
def round_pmf_vector(ls):
    ls = ls.round(6)
    ls = np.clip(ls, a_min=1e-6, a_max=1 - 1e-6)
    ll = len(ls)
    for i in range(ll):
        delta = 1.000000 - sum(ls)
        if ls[i] > 2 * abs(delta):
            ls[i] += delta
            break
    return ls
...
def obtain_posterior(data_train, args_dict, model=None):
    """
    Turing.jl impmementation of HMM prediction.
    Reference: https://turing.ml/dev/tutorials/4-bayeshmm/
    ...
    jl_filename = os.path.join(
        sys.path[0], "ppls/turingjl/hiddenMarkov" + str(bool(observe_model)) + ".jl"
    )
    ...
    if observe_model:
        xs = posterior_samples.x[-1]
        samples_formatted = []
        for xn1 in xs:
            samples_formatted.append({xn1str: xn1[0] - 1})

    else:
        thetas, mus, sigmas, xs = posterior_samples
        thetas = thetas.T
        mus = mus.mu
        sigmas = sigmas.sigma
        xs = xs.x
        ...
        thetas = [square_matrix(theta, K) for theta in thetas]
