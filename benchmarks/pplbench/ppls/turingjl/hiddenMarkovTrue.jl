...
function main(N, K, concentration, mu_loc, mu_scale, sigma_shape, sigma_scale, num_samples, observations, T, mu, sigma)
    ...
    @model BayesHmm(y, K, N, mu_loc, mu_scale, sigma_shape, sigma_scale, T, mu, sigma) = begin
        # State sequence.
        x = tzeros(Int, N)
    ...
    g = PG(20, :x)
    chain = sample(BayesHmm(observations, K, N, mu_loc, mu_scale, sigma_shape, sigma_scale, T, mu, sigma), g, num_samples);
