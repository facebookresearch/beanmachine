...
By adding `reproduction_rate()` to the `queries` keyword, we ran inference to compute the posterior for that random variable. We can see that it, and no other random variable, is available in `samples`:
...
In the case of our example model, we have a healthy $\hat{R}$ value very close to 1.0, and a healthy relative number of effective samples: around 98% and 93% in the bulk and tail estimators.
