...
<BokehFigure data={React.useMemo(() => require("./posterior_rate_dynamic.json"), [])} />
...
<BokehFigure data={React.useMemo(() => require("./mcmc_trace.json"), [])} />

```py
az.plot_autocorr({"Reproduction rate": samples[reproduction_rate()]})
```

<BokehFigure data={React.useMemo(() => require("./mcmc_autocorr.json"), [])} />
