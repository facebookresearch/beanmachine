...
nmc_proposer(const graph::NodeValue& value, double grad1, double grad2) {
  ...
  if (value.type == graph::AtomicType::REAL) {
    ...
    // first a random walk
    weights.push_back(RANDOM_WALK_WEIGHT);
    proposers.push_back(std::make_unique<Normal>(x.item().toDouble(), 1.0));
    ...
    if (is_valid_grad and grad2 < 0) {
      ...
      double mu = value._value.item().toDouble() - grad1 / grad2;
  ...
  //   // first a random walk
  //   weights.push_back(RANDOM_WALK_WEIGHT);
  //   proposers.push_back(std::make_unique<TruncatedCauchy>(value._value, 1.0));
