...
TEST(testdistrib, cauchy) {
  ...
  const double X0 = 1.2;
  auto scale_real = g.add_constant(SCALE);
  auto x0_real = g.add_constant(X0);
  auto pos1 = g.add_constant_pos_real(SCALE);
  // negative tests: cauchy has two parents which are reals
  EXPECT_THROW(
      g.add_distribution(
  ...
  EXPECT_THROW(
      g.add_distribution(
          DistributionType::CAUCHY,
          AtomicType::REAL,
          std::vector<uint>{pos1, scale_real}),
      std::invalid_argument);
  EXPECT_THROW(
      g.add_distribution(
          DistributionType::CAUCHY,
          AtomicType::REAL,
          std::vector<uint>{x0_real, pos1}),
      std::invalid_argument);
  ...
  auto scale =
      g.add_operator(OperatorType::ADD, std::vector<uint>{zero, scale_real});
