...
TEST(testdistrib, beta1) {
  ...
  EXPECT_THROW(
      g1.add_distribution(DistributionType::BETA, AtomicType::REAL, std::vector<uint>{pos1, pos2}),
      std::invalid_argument);
  // negative tests gamma has two parents
  EXPECT_THROW(
      g1.add_distribution(DistributionType::BETA, AtomicType::PROBABILITY, std::vector<uint>{}),
      std::invalid_argument);
  EXPECT_THROW(
      g1.add_distribution(DistributionType::BETA, AtomicType::PROBABILITY, std::vector<uint>{pos1}),
      std::invalid_argument);
  // negative test the parents must be both positive real.
  EXPECT_THROW(
      g1.add_distribution(DistributionType::BETA, AtomicType::PROBABILITY, std::vector<uint>{neg1, pos1}),
      std::invalid_argument);
  ...
  EXPECT_NEAR(means[1] - means[0] * means[0], A * B/ ((A + B) * (A + B) * (A + B + 1)), 0.01);
  // test log_prob
  g1.observe(x, 0.2);
  ...
  // y ~ Beta(a^2, b^2)
  Graph g2;
  auto a = g2.add_operator(OperatorType::SAMPLE, std::vector<uint>{
      g2.add_distribution(DistributionType::FLAT, AtomicType::POS_REAL, std::vector<uint>{})});
  auto a_sq = g2.add_operator(OperatorType::MULTIPLY, std::vector<uint>{a, a});
  g2.observe(a, 1.5);
  auto b = g2.add_operator(OperatorType::SAMPLE, std::vector<uint>{
      g2.add_distribution(DistributionType::FLAT, AtomicType::POS_REAL, std::vector<uint>{})});
  auto b_sq = g2.add_operator(OperatorType::MULTIPLY, std::vector<uint>{b, b});
  g2.observe(b, 2.0);
  auto y = g2.add_operator(OperatorType::SAMPLE, std::vector<uint>{
      g2.add_distribution(DistributionType::BETA, AtomicType::PROBABILITY,
  ...
  // f_grad_a[0].item(), f_grad2_a[0].item() # -> -0.07819676399230957, -3.506779909133911
  // f_grad_b = torch.autograd.grad(f_x, b, create_graph=True)
  // f_grad2_b = torch.autograd.grad(f_grad_b, b)
  // f_grad_b[0].item(), f_grad2_b[0].item() # -> 0.5506436824798584, -1.4901777505874634
...
TEST(testdistrib, beta2) {
  ...
  auto x = g1.add_operator(OperatorType::IID_SAMPLE, std::vector<uint>{beta_dist, two});
  g1.query(x);
  ...
  // a = torch.tensor([1.5], requires_grad=True)
  // b = torch.tensor([2.0], requires_grad=True)
  // f_x = torch.distributions.Beta(a**2, b**2).log_prob(tensor([0.3, 0.4])).sum()
  // f_grad_a = torch.autograd.grad(f_x, a, create_graph=True)
  // f_grad2_a = torch.autograd.grad(f_grad_a, a)
  // f_grad_a[0].item(), f_grad2_a[0].item() # -> 0.7066526412963867, -6.438195705413818
  // f_grad_b = torch.autograd.grad(f_x, b, create_graph=True)
  // f_grad2_b = torch.autograd.grad(f_grad_b, b)
  // f_grad_b[0].item(), f_grad2_b[0].item() # -> 0.48468494415283203, -3.288656711578369
