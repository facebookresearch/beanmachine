...
compute_support_affected_nodes_except_self(Graph& graph, uint node_id);
...
void marginalize_graph(Graph& graph, uint discrete_sample_node_id) {
  ...
  std::tie(det_node_ids, sto_node_ids) =
      compute_support_affected_nodes_except_self(graph, discrete_sample->index);
...
std::tuple<std::vector<uint>, std::vector<uint>>
compute_support_affected_nodes_except_self(Graph& graph, uint node_id) {
  std::set<uint> support = graph.compute_support();
  return graph.compute_affected_nodes_except_self(node_id, support);
}
