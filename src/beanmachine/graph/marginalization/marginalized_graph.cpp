...
void _connect_parent_to_marginal_distribution(
...
void marginalize_graph(Graph& g, uint discrete_sample_node_id) {
  Node* discrete_sample = g.get_node(discrete_sample_node_id);
  Node* discrete_distribution = discrete_sample->in_nodes[0];

  std::unique_ptr<SubGraph> subgraph_ptr = std::make_unique<SubGraph>(g);
  ...
  std::set<uint> supp_ids = g.compute_full_ordered_support_node_ids();
  std::vector<uint> det_node_ids;
  std::vector<uint> sto_node_ids;
  std::tie(det_node_ids, sto_node_ids) =
      g.compute_children(discrete_sample->index, supp_ids);
  ...
  for (uint sto_node_id : sto_node_ids) {
    Node* sto_node = g.get_node(sto_node_id);
  ...
  for (uint id : sto_node_ids) {
    ...
    _move_children_if(node, copy_node.get(), [&](Node* child) {
  ...
  for (Node* parent : marginal_distribution->in_nodes) {
    ...
    _move_children_if(parent, copy_node.get(), [&](Node* child) {
      return subgraph->has_node(child->index);
    });
  ...
  for (uint i = 0; i < marginal_distribution_and_copy_chidren.size(); i++) {
    g.nodes.insert(
        std::next(g.nodes.begin(), marginal_distribution_index + i),
