...
def cmake_build_release(src_dir:str, build_dir:str, cmake_args:List[str], build_args:List[str], name:str):
    if os.path.isdir(build_dir):
        print(f"Skipping {name} build. Delete build directory if you want to rebuild it.")
        return
    ...
    cmake_result = subprocess.run(["cmake", src_dir] + cmake_args, cwd=build_dir, check=False, capture_output=True)
    if cmake_result.returncode != 0:
        print(str(cmake_result.stdout))
        print(str(cmake_result.stderr))
    build_result = subprocess.run(build_args, cwd=build_dir, check=True, capture_output=True)
...
def build_llvm(src_dir:str, cmake_build_dir: str):
    if not os.path.isdir(cmake_build_dir):
        ...
        cmake_build_release(llvm_dir, cmake_build_dir, cmake_args, ["cmake", "--build", ".", "--target", "check-mlir"], "llvm")

def build_pybind11(base_dir:str):
    pybind11_dir = os.path.join(base_dir, "src", "beanmachine", "paic2", "externals", "pybind11")
    build_dir = os.path.join(pybind11_dir, "build")
    cmake_args = [f"-DPYTHON_EXECUTABLE={sys.executable}"]
    cmake_build_release(pybind11_dir, build_dir, cmake_args, ["cmake", "--build", "."], "pybind11")

def build_paic2(base_dir:str, paic2_src: str, paic2_build_dir: str, cmake_module_path: str):
    ...
    cmake_build_release(paic2_src, paic2_build_dir, paic2_cmake_args, ["cmake", "--build", "."], "paic2")
