...
class BayesianDiffInDiff:
    def __init__(
        ...
        Note: In a future version, num_features could be added as a user-defined parameter if/when there is added
        support for additional covariates.
        """
        self._is_fit = False
        self._posterior_samples = pd.DataFrame()
        self._diagnostics = pd.DataFrame()
        self._num_features = 3
    ...
    def plot_parallel_trends_assumption(self) -> matplotlib.figure.Axes:
        ...
        Returns
        -------
        type: matplotlib.figure.Axes
            object representing the data plotted to assist with checking for
    ...
    def fit(
        ...
        Returns
        -------
        type: BayesianDiffInDiff
            Returns Self to allow for fluid method chaining (i.e. model.fit().get_treatment_effect())
        """
    ...
    def get_posterior_samples(self) -> pd.DataFrame:
        """
        Returns posterior samples of model in a pd.DataFrame.
        Will raise a ModelNotFitException() if model is not fit() first.
        """
        if not self._is_fit:
            raise ModelNotFitException("Please fit model first.")
    ...
    def get_diagnostics(self) -> pd.DataFrame:
        """
        Returns diagnostics of model in a pd.DataFrame.
        Will raise a ModelNotFitException() if model is not fit() first.
        """
        if not self._is_fit:
            raise ModelNotFitException("Please fit model first.")
