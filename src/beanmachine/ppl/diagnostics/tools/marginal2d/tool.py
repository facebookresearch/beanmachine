...
from beanmachine.ppl.diagnostics.tools.marginal2d import utils
...
class Marginal2d(DiagnosticToolBaseClass):
    """
    ...
    def __init__(self: Marginal2d, mcs: MonteCarloSamples) -> None:
        super(Marginal2d, self).__init__(mcs)

    def create_document(self: Marginal2d) -> Model:
        # Initialize widget values using Python.
        rv_name_x = self.rv_names[0]
        rv_name_y = self.rv_names[0]
        bw_factor = 1.0
        bandwidth_x = 1.0
        bandwidth_y = 1.0
        ...
        figures = utils.create_figures(rv_name_x=rv_name_x, rv_name_y=rv_name_y)

        # Create empty glyphs and attach them to the figures using Python.
        glyphs = utils.create_glyphs()
        utils.add_glyphs(figures=figures, glyphs=glyphs, sources=sources)
        ...
        callback_js = f"""
            const rvNameX = widgets.rv_select_x.value;
            const rvDataX = data[rvNameX];
            const rvNameY = widgets.rv_select_y.value;
            const rvDataY = data[rvNameY];
            const hdiProbabilityX = widgets.hdi_slider_x.value / 100;
            const hdiProbabilityY = widgets.hdi_slider_y.value / 100;
            const bwFactor = widgets.bw_factor_slider.value;
            // Remove the CSS classes that dim the tool output on initial load.
            const rvX = widgets.rv_select_x.value;
            const rvY = widgets.rv_select_y.value;
            const rvXCheck = rvX !== 'Select a random variable...';
            const rvYCheck = rvY !== 'Select a random variable...';
            const cssRemovalCheck = rvXCheck && rvYCheck;
            if (cssRemovalCheck) {{
            ...
            try {{
              const [bandwidthX, bandwidthY] = marginal2d.update(
                rvDataX,
                hdiProbabilityX,
                rvDataY,
                hdiProbabilityY,
            ...
            figures.xy.y_range = figures.x.y_range;
        """
        ...
        rv_select_x_js = f"""
            widgets.bw_factor_slider.value = 1.0;
            widgets.hdi_slider_x.value = 89;
            widgets.hdi_slider_y.value = 89;
            {callback_js};
        """
        rv_select_x_callback = CustomJS(args=callback_arguments, code=rv_select_x_js)
        rv_select_y_js = f"""
            widgets.bw_factor_slider.value = 1.0;
            widgets.hdi_slider_x.value = 89;
            widgets.hdi_slider_y.value = 89;
            {callback_js};
        """
        rv_select_y_callback = CustomJS(args=callback_arguments, code=rv_select_y_js)
        slider_js = f"""
            {callback_js};
        """
        ...
        widgets["rv_select_x"].js_on_change("value", rv_select_x_callback)
        widgets["rv_select_y"].js_on_change("value", rv_select_y_callback)
        widgets["bw_factor_slider"].js_on_change("value", slider_callback)
        widgets["hdi_slider_x"].js_on_change("value", slider_callback)
        widgets["hdi_slider_y"].js_on_change("value", slider_callback)
